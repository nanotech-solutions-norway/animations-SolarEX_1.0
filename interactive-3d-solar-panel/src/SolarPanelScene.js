...
root.name = 'SolarEX-selected-center-four-model';
...
[-3.15, 0, 3.15].forEach((x, i) => {
  box(panel, `rear-vertical-rail-${i}`, 0.14, H * 0.76, 0.17, x, -0.04, -0.84, mat.metal);
  box(panel, `upper-clamp-${i}`, 0.36, 0.18, 0.22, x, 1.94, -0.92, mat.bolt);
  box(panel, `lower-clamp-${i}`, 0.36, 0.18, 0.22, x, -2.03, -0.92, mat.bolt);
});
...
function cable(name, xOffset, side = 1) {
  const curve = new THREE.CatmullRomCurve3([
    new THREE.Vector3(xOffset, 0.8, -1.18), new THREE.Vector3(xOffset + side * 0.18, 0.5, -1.28),
    new THREE.Vector3(xOffset + side * 0.78, 0.02, -1.34), new THREE.Vector3(xOffset + side * 1.56, -0.58, -1.22),
...
cable('left-output-cable', -0.18, -1);
cable('right-output-cable', 0.18, 1);
...
function setKeyframe(i) { const k = keyframes[i]; camera.position.set(...k.pos); root.rotation.set(...k.rot); camera.lookAt(0, 0.75, 0); sceneLabel.textContent = k.name; }
...
canvas.addEventListener('pointermove', (e) => { if (!drag) return; const dx = e.clientX - lastX, dy = e.clientY - lastY; lastX = e.clientX; lastY = e.clientY; targetRotY += dx * 0.006; targetRotX = THREE.MathUtils.clamp(targetRotX + dy * 0.0034, -0.72, 0.6); });
['pointerup', 'pointercancel'].forEach((type) => canvas.addEventListener(type, (e) => { drag = false; canvas.releasePointerCapture?.(e.pointerId); }));
playBtn.addEventListener('click', () => { cinematic = true; cinematicStart = performance.now(); sceneLabel.textContent = 'Animation_scenen_001'; });
...
window.addEventListener('resize', resize); resize(); setKeyframe(0);
function animate(time) { requestAnimationFrame(animate); if (cinematic) applyCinematic(time); else { if (autoRotateInput.checked && !reducedMotion) targetRotY += 0.0022; root.rotation.x += (targetRotX - root.rotation.x) * 0.075; root.rotation.y += (targetRotY - root.rotation.y) * 0.075; root.rotation.z += (targetRotZ - root.rotation.z) * 0.075; camera.lookAt(0, 0.75, 0); } renderer.render(scene, camera); }
